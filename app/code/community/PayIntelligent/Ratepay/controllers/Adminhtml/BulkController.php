...
class PayIntelligent_Ratepay_Adminhtml_BulkController extends Mage_Adminhtml_Controller_Action
{
    
    /**
     * Initialize bulk view
     * 
     * @return PayIntelligent_Ratepay_Adminhtml_BulkController
     */
    ...
    private function _ratepayMassEvent($type)
    {
        ...
        foreach ($this->getRequest()->getParam('order_id') as $orderId) {
            ...
            if ($this->_isEventPossible($type, $order)) {
                ...
                $msg = Mage::helper('ratepay')->__('The order with the number %s can not be '. $type . 'ed.<br/>', $order->getIncrementId());
            }
        }
        ...
        if ($counter > 0) {
            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('ratepay')->__('Total of %d record(s) were  '. $type . 'ed.', $counter));
        }
    }
     ...
     * @throws Exception Pi Wrong operation!
     */
    private function _ratepayEvent($type, $order)
    {
        switch($type){
            ...
            case 'cancel':
                ...
                break;
            default:
                throw new Exception('Pi Wrong operation!');
        }
    }
