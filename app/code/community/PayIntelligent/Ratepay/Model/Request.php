...
class PayIntelligent_Ratepay_Model_Request extends Mage_Core_Model_Abstract
{
    ...
    private function _setRequestVersions($head)
    {
        ...
        $system->addAttribute('name', 'Magento_' . Mage::helper('ratepay')->getEdition());
        $system->addAttribute('version',  Mage::getVersion() . '_' . Mage::getStoreConfig('payment/ratepay/version'));
    }
    ...
    private function sendXmlRequest($loggingInfo)
    {
        ...
        $client = Mage::getSingleton('ratepay/request_communication', array($sandbox));
    ...
    public function constructXml()
    {
        ...
        $this->request = null;
        $this->request = new PayIntelligent_Ratepay_Model_Request_Xml($xmlString);
    }
